...
test('a brief can reference its source thread', function () {
    ...
    expect($brief->sourceThread)->toBeInstanceOf(Thread::class)
        ->and($brief->sourceThread->is($thread))->toBeTrue()
...
test('a brief can exist without a source thread', function () {
    ...
    expect($brief->sourceThread)->toBeNull();
});

test('a brief stores category and acceptance criteria', function () {
    $brief = Brief::factory()->create([
...
test('brief acceptance criteria defaults to an empty array', function () {
    $brief = Brief::create([
    ...
    expect($brief->acceptance_criteria)->toBe([]);
});
