...
new #[Layout('layouts::workspace'), Title('Briefs')] class extends Component
{
    ...
    public function mount(?Brief $brief = null): void
    {
        if ($brief) {
            abort_unless($brief->workspace_id === $this->workspace()?->id, 404);
            $this->selectBrief($brief->id);

            return;
        }

        if (request()->routeIs('briefs.create')) {
            $this->startNewBrief();
        ...
        <div class="flex items-center justify-between gap-3 border-b border-neutral-200 px-4 py-3 dark:border-white/10">
            ...
            @if ($selectedBriefId)
                <div class="flex shrink-0 gap-2">
                    ...
                    <flux:button :href="route('briefs.show', $selectedBriefId)" wire:navigate size="xs" variant="filled" icon="eye" data-test="view-brief-button">
                        {{ __('View') }}
                    </flux:button>
